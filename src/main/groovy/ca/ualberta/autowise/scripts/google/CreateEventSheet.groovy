...
private def static generateSingleStaticUpdates(Event e){
    ...
    result.add(new ValueRange().setRange(EventSlurper.staticSingleValues.get("campaignStartOffset")).setValues([[TimeUnit.MILLISECONDS.toDays( e.campaignStartOffset).toString()]]))
    ...
    result.add(new ValueRange().setRange(EventSlurper.staticSingleValues.get("followupOffset")).setValues([[TimeUnit.MILLISECONDS.toHours(e.followupOffset).toString()]]))
