...
static def confirmationEmailTask(Vertx vertx, services, Task task, config, subject){
    ...
    sender.sendMassEmail((rowData, rowFuture)->{
        ...
        if(rowData.get(2).equals("Accepted")){
            ...
            emailContents = emailContents.replaceAll("%CONFIRMATION_LINK%", "<a href=\"http://${config.getString("host")}:${config.getInteger("port").toString()}/${confirmHook.path()}\">Confirmation Link</a>")
            emailContents = emailContents.replaceAll("%EVENTBRITE_LINK%", "<a href=\"${eventbriteLink}\">eventbrite</a>")
            emailContents = emailContents.replaceAll("%CANCEL_LINK%", "<a href=\"http://${config.getString("host")}:${config.getInteger("port").toString()}/${cancelHook.path()}\">Cancellation Link</a>")
